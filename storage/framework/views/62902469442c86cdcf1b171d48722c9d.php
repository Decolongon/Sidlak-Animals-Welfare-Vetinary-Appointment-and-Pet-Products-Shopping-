...
[$__name, $__params] = $__split('adoption.adoption-cart-counter');

$__html = app('livewire')->mount($__name, $__params, 'lw-2299266832-0', $__slots ?? [], get_defined_vars());
...
?>
                ...
                <?php if(auth()->guard()->guest()): ?>
                    <a  href="<?php echo e(route('filament.auth.auth.login')); ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-black border border-gray-200 gap-x-2 rounded-xl hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:border-neutral-700 dark:hover:bg-white/10 dark:text-white dark:hover:text-white">
                        <?php echo e(__('Sign in')); ?>

                    </a>

                    <?php if(Route::has('register')): ?>
                    <a  href="<?php echo e(route('filament.auth.auth.register')); ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-black transition border border-transparent gap-x-2 rounded-xl bg-amber-400 hover:bg-amber-500 disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:bg-amber-500">
...
[$__name, $__params] = $__split('ecommerce.cart-count');

$__html = app('livewire')->mount($__name, $__params, 'lw-2299266832-1', $__slots ?? [], get_defined_vars());
                    ...
                    <div class="relative inline-flex hs-dropdown" data-hs-dropdown-placement="bottom-right">
                        <?php if(Laravel\Jetstream\Jetstream::managesProfilePhotos()): ?>
                            <button class="flex text-sm transition border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300">
                                <img class="object-cover w-8 h-8 rounded-full" src="<?php echo e(Auth::user()->profile_photo_url); ?>" alt="" />
                            </button>
                        <?php else: ?>
                            <span class="inline-flex rounded-md">
                                <button type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium leading-4 text-black transition duration-150 ease-in-out border border-transparent rounded-md bg-amber-400 hover:bg-amber-500 hover:text-gray-900 focus:outline-none focus:bg-amber-500 active:bg-amber-500">
                                    <?php echo e(Str::before(Auth::user()->name, ' ')); ?>
                        ...
                        <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 z-10 bg-white shadow-md rounded-lg p-2 dark:bg-neutral-800 dark:border dark:border-neutral-700" aria-labelledby="hs-dropdown-with-header">
                            <div class="px-5 py-3 -m-2 bg-gray-100 rounded-t-lg dark:bg-neutral-700">
                                <p class="text-sm text-gray-500 dark:text-neutral-400"><?php echo e(__('Manage Menu')); ?></p>
                                <p class="text-sm font-medium text-gray-800 dark:text-neutral-300"><?php echo e(Auth::user()->email); ?></p>
                            </div>
                        <div class="py-2 mt-2 first:pt-0 last:pb-0">

                            <a wire:navigate class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-amber-500 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300"  href="<?php echo e(route('profile.show')); ?>">
                            ...
                            <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-amber-500 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300" href="<?php echo e(route('api-tokens.index')); ?>">
                                <svg class="flex-shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                            ...
                            <div>
                                ...
                                <form method="POST" action="<?php echo e(route('custom-logout')); ?>" x-data>
                                    <?php echo csrf_field(); ?>
                                    <a href="#" 
                                    class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-amber-500 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300" 
...
<?php endif; ?>
                <?php if (isset($component)) { $__componentOriginalc295f12dca9d42f28a259237a5724830 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginalc295f12dca9d42f28a259237a5724830 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.nav-link','data' => ['wire:navigate' => true,'href' => ''.e(route('page.dogs')).'','active' => request()->routeIs('page.dogs') || request()->routeIs('page.dog.single')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('nav-link'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['wire:navigate' => true,'href' => ''.e(route('page.dogs')).'','active' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(request()->routeIs('page.dogs') || request()->routeIs('page.dog.single'))]); ?><?php echo e(__('Dogs')); ?> <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginalc295f12dca9d42f28a259237a5724830)): ?>
<?php $attributes = $__attributesOriginalc295f12dca9d42f28a259237a5724830; ?>
<?php unset($__attributesOriginalc295f12dca9d42f28a259237a5724830); ?>
...
<?php /**PATH C:\laragon\www\sidlak-vet-appointment-and-pet-shop\resources\views/navigation-menu.blade.php ENDPATH**/ ?>