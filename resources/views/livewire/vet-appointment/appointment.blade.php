   ...
   @if ($schedules['schedules'] && $schedules['appointmentsCount'] < $schedules['schedules']->num_customers)
    ...
    <form wire:submit="submit" >
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        ...
        <!-- user_id -->
        {{-- <div>
          <label for="user_id" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">User ID</label>
      ...
      " wire:ignore>
          ...
          >
              @foreach ($appointmentCategories as $category)
                  <option value="{{ $category->id }}">{{ ucwords($category->appoint_cat_name) }}</option>
              @endforeach
          </select>
