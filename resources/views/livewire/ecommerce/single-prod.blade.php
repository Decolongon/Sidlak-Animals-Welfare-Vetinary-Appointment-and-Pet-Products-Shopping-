    ...
    <div class="bg-white dark:bg-[#262626] p-6 rounded-lg shadow-md mt-10">
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            @endif
                ...
                <p class="{{ $product->prod_quantity > 10 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }} mt-2">
        ...
        @if ($product && $product->images->count() > 1)
            <div data-hs-carousel='{
                ...
                <button type="button" class="hs-carousel-prev hs-carousel-disabled:opacity-50 hs-carousel-disabled:pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center w-11.5 h-full text-gray-800 hover:bg-gray-800/10 focus:outline-none focus:bg-gray-800/10 dark:text-white dark:hover:text-blue-400 rounded-s-lg">
                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="m15 18-6-6 6-6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </button>
                <button type="button" class="hs-carousel-next hs-carousel-disabled:opacity-50 hs-carousel-disabled:pointer-events-none absolute inset-y-0 end-0 flex items-center justify-center w-11.5 h-full text-gray-800 hover:bg-gray-800/10 focus:outline-none focus:bg-gray-800/10 dark:text-white dark:hover:text-blue-400 rounded-e-lg">
                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mt-6 border-t border-gray-400 dark:border-gray-700 pt-4">
            ...
            <div class="flex space-x-6 text-black dark:text-white font-medium">
                <a href="#" wire:click.prevent="$set('activeTab', 'description')" 
                   :class="{'border-b-2 border-black dark:border-white pb-2': $wire.activeTab === 'description', 'text-gray-500 dark:text-gray-400': $wire.activeTab !== 'description'}">
                    Description
                </a>
                <a href="#" wire:click.prevent="$set('activeTab', 'reviews')" 
                   :class="{'border-b-2 border-black dark:border-white pb-2': $wire.activeTab === 'reviews', 'text-gray-500 dark:text-gray-400': $wire.activeTab !== 'reviews'}">
                    Reviews (<span>{{ $count_reviews }}</span>)
                </a>
                @if (Auth::check())
                    <a href="#" wire:click.prevent="$set('activeTab', 'post_reviews')" 
                       :class="{'border-b-2 border-black dark:border-white pb-2': $wire.activeTab === 'post_reviews', 'text-gray-500 dark:text-gray-400': $wire.activeTab !== 'post_reviews'}">
                        Post Reviews Here
                    </a>
                @endif
            </div>
 ...
 @if($activeTab == 'description')   
    ...
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto scroll-visible">
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4">
            @forelse($relatedProducts as $product)
                <div wire:key="product-{{ $product->id }}" class="group flex flex-col border border-gray-200 hover:border-transparent hover:shadow-lg focus:outline-none focus:border-transparent focus:shadow-lg transition duration-300 rounded-xl p-4 dark:border-neutral-700 dark:hover:border-transparent dark:hover:shadow-black/40 dark:focus:border-transparent dark:focus:shadow-black/40">
                    ...
                    <p class="text-xs flex justify-between items-center gap-x-2 text-green-500 dark:text-green-400 mb-2">
                        ...
                        @endif
                        <span class="{{ $product->prod_quantity > 10 ? 'text-green-500' : 'text-red-500' }}">
                    ...
                    <div class="flex flex-col flex-grow mt-4">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-x-4 overflow-hidden max-w-full mt-2">
                            <p class="text-sm font-semibold text-gray-800 dark:text-neutral-300">
                                @if($product->discounted_price !== null)
                                    <del class="text-gray-500 dark:text-neutral-400">
                                        ₱{{ number_format($product->prod_price, 2) }}
                                    ...
                                    <span class="text-green-600 font-semibold ml-2">{{ $product->discount_label }}</span>
                                @else
                                    ₱{{ number_format($product->prod_price, 2) }}
                                @endif
                            </p>
            ...
            @empty
                <div class="col-span-full text-center py-10">
                    <p class="text-gray-500 dark:text-neutral-400 text-lg">No Related Products Available.</p>
                </div>
            @endforelse
        </div>
    </div>
