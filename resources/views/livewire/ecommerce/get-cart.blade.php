                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <input type="checkbox"
                                ...
                                name="selectedItems" wire:model="selectedItems" value="{{ $cart->id }}">
                            ...
                            <div wire:ignore
                                ...
                                @if (isset($cart->variant))
                                    <img src="{{ asset(Storage::url($cart->variant->url)) }}"
                                        alt="{{ $cart->product->prod_slug }}" class="object-cover w-full h-full">
                                @else
                                    @if ($cart->product->primary_image && $cart->product->primary_image->url)
                                        <img src="{{ asset(Storage::url($cart->product->primary_image->url)) }}"
                                            alt="{{ $cart->product->prod_slug }}" class="object-cover w-full h-full">
                                    @endif
                                @endif
                            </div>
                            <div>
                                <div class="font-medium text-gray-800 dark:text-neutral-200" wire:ignore>
                                    ...
                                    @if ($cart->product->prod_unit === 'kg' || $cart->product->prod_unit === 'g')
                                        <span class="text-sm text-gray-600 dark:text-neutral-400 whitespace-nowrap">
                                            {{ number_format($cart->product->prod_weight, 2) }}
                                            {{ $cart->product->prod_unit }}
                                        </span>
                                    @endif
                                </div>
                                <div class="text-sm text-gray-600 dark:text-neutral-400">

                                    @if ($cart->product->discounted_price !== null && $cart->product->prod_unit !== 'diff_size')
                                    ...
                                    ₱{{ number_format($cart->product->prod_price, 2) }}
                                        ...
                                        <small
                                            class="text-success d-block">{{ $cart->product->discount_label }}</small>
                                    @else
                                        @if (isset($cart->variant))
                                            ₱{{ number_format($cart->quantity * $cart->variant->price, 2) }}
                                        @else
